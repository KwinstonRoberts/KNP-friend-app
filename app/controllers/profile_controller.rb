class ProfileController < ApplicationController
  ...
  def get_matches
    return User.includes(result: [:personalities,:traits]).where('traits.name=? and personalities.name',@traits.name, @user.result.personalities.first).references(:personalities,:traits)
  end
end
